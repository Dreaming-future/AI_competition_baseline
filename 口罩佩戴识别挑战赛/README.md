...
```bash
CUDA_VISIBLE_DEVICES=1 python train.py -f --cuda --net EfficientNetv2-XL --num-workers 8 --epochs 50 -fe 50
```
...
现在排行榜并列第一，达到准确率有0.98095的水准

具体方法是利用多个模型集成，并且利用了vote的投票方式得到最后的结果。

|  ID  |   状态   |  评分   |        提交文件名        |                           提交备注                           |      提交者       |      提交时间       |
| :--: | :------: | :-----: | :----------------------: | :----------------------------------------------------------: | :---------------: | :-----------------: |
|  1   | 返回分数 | 0.98095 |   submit_Ensemble.csv    | 利用多个预训练模型进行微调，都迭代了大概50次，只修改分类层，然后利用集成模型的投票法进行预测得到 | 擅长射手的pikachu | 2022-07-06 23:16:12 |
|  2   | 返回分数 | 0.95873 |  submit_ConvNeXt-XL.csv  | 利用ImageNet1k数据集的均值以后，进行ConvNeXt的训练，总共迭代61次。 | 擅长射手的pikachu | 2022-07-05 23:43:44 |
|  3   | 返回分数 | 0.96905 |  submit_ConvNeXt-XL.csv  |  修改了数据集的均值以后，进行ConvNeXt的训练，总共迭代45次。  | 擅长射手的pikachu | 2022-07-05 12:22:09 |
|  4   | 返回分数 | 0.96508 |  submit_ConvNeXt-XL.csv  | 修改了数据集的均值以后，进行ConvNeXt的训练，总共迭代45次，得到最优结果99.96%左右 | 擅长射手的pikachu | 2022-07-05 12:18:12 |
|  5   | 返回分数 | 0.96349 |  submit_ConvNeXt-L.csv   | 试一下ConvNeXt-L的结果，固定了卷积层的数目得到的结果，只进行微调，均值方差不同了 | 擅长射手的pikachu | 2022-07-04 09:20:45 |
|  6   | 返回分数 | 0.97222 |      submit_XL.csv       | 尝试一下ConvNeXt-XL的结果，固定了卷积层的数目得到的结果，只进行微调 | 擅长射手的pikachu | 2022-07-04 09:19:26 |
|  7   | 返回分数 | 0.96032 |   submit_Ensemble.csv    | 使用多个集成模型投票，有ConvNeXt-T-B-L,DenseNet169,ViT,Swin  | 擅长射手的pikachu | 2022-07-04 09:18:33 |
|  8   | 返回分数 | 0.96905 |       submit_L.csv       | 利用ConvNeXt-L进行训练50次，固定了卷积层的数目得到的结果，只进行微调 | 擅长射手的pikachu | 2022-07-03 09:24:29 |
|  9   | 返回分数 | 0.96905 | submit_convnext_base.csv | 利用ConvNeXt-B进行训练50次，固定了卷积层的数目得到的结果，只进行微调 | 擅长射手的pikachu | 2022-07-03 02:25:00 |
|  10  | 返回分数 | 0.88571 |  submit_densenet201.csv  |      尝试使用DenseNet201进行训练迭代50次，得到结果测试       | 擅长射手的pikachu | 2022-07-03 02:24:36 |
|  11  | 返回分数 | 0.95317 |       submit3.csv        |   利用ConvNeXt-T进行训练20次，固定了卷积层的数目得到的结果   | 擅长射手的pikachu | 2022-07-02 23:41:35 |
|  12  | 返回分数 | 0.95952 |       submit2.csv        |               利用DenseNet169进行计算迭代50次                | 擅长射手的pikachu | 2022-07-02 21:18:10 |
|  13  | 返回分数 | 0.83016 |        submit.csv        |                   MobieNetv2进行测试的结果                   | 擅长射手的pikachu | 2022-07-02 17:51:18 |
